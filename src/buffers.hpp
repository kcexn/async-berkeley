...
public:
  ...
  sockbuf();
  explicit sockbuf(native_handle_type sockfd, bool connected = false,
                   std::ios_base::openmode which = (std::ios_base::in |
                                                    std::ios_base::out));
  explicit sockbuf(int domain, int type, int protocol,
                   std::ios_base::openmode which = (std::ios_base::in |
                                                    std::ios_base::out));
  auto connectto(const struct sockaddr *addr, socklen_t addrlen) -> buffer_type;

  [[nodiscard]] auto recvbuf() const -> const buffer_type & {
    return _buffers.front();
  }
  [[nodiscard]] auto sendbuf() const -> const buffer_type & {
...
protected:
  auto seekoff(off_type off, std::ios_base::seekdir dir,
               std::ios_base::openmode which =
                   std::ios_base::in | std::ios_base::out) -> pos_type override;
  auto seekpos(pos_type pos, std::ios_base::openmode which = std::ios_base::in |
                                                             std::ios_base::out)
      -> pos_type override;
...
private:
  ...
  void _resizewbuf(const buffer_type &buf, void *iov_base = nullptr,
                   std::size_t buflen = 0);
  void _memmoverbuf();
  auto _recv() -> int;
};
