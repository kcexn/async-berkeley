...
static void throw_system_error(const std::string &what) {
  throw std::system_error(std::error_code(errno, std::system_category()), what);
}
...
sockbuf::sockbuf()
    : Base(), _buffers{}, _socket{BAD_SOCKET}, _errno{0}, _connected{false},
      _which{std::ios_base::in | std::ios_base::out} {
  _init_buf_ptrs();
}

sockbuf::sockbuf(native_handle_type sockfd, bool connected,
                 std::ios_base::openmode which)
    : Base(), _buffers{}, _socket{sockfd}, _errno{0}, _connected{connected},
      _which{which} {
  _init_buf_ptrs();
}

sockbuf::sockbuf(int domain, int type, int protocol,
                 std::ios_base::openmode which)
    : Base(), _buffers{}, _socket{BAD_SOCKET}, _errno{0}, _connected{false},
      _which{which} {
...
sockbuf::buffer_type sockbuf::connectto(const struct sockaddr *addr,
                                        socklen_t addrlen) {
...
sockbuf::pos_type sockbuf::seekoff(off_type off, std::ios_base::seekdir dir,
                                   std::ios_base::openmode which) {
  pos_type pos = 0;
  switch (dir) {
...
sockbuf::Base::pos_type sockbuf::seekpos(Base::pos_type pos,
                                         std::ios_base::openmode which) {
  if (pos < 0)
...
void sockbuf::_resizewbuf(const buffer_type &buf, void *base,
                          std::size_t buflen) {
  auto &sendbuf_ = buf->data;
  if (base) {
    if (buflen)
...
int sockbuf::_send(const buffer_type &buf) {
  ...
  if ((_socket == BAD_SOCKET) ||
      (!_connected && address.ss_family == AF_UNSPEC)) {
    _resizewbuf(buf);
    return 0;
  } else if (_connected ||
  ...
  ssize_t len = 0;
  while (!(_errno = 0) &&
         (len = sendmsg(_socket, &header, MSG_DONTWAIT | MSG_NOSIGNAL))) {
    if (len > 0) {
