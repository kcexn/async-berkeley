...
public:
  ...
  sockstream() : Base(&_buf), _buf{} {}
  explicit sockstream(native_handle_type sockfd, bool connected = false,
                      std::ios_base::openmode which = (std::ios_base::in |
                                                       std::ios_base::out))
      : Base(&_buf), _buf(sockfd, connected, which) {}
  explicit sockstream(int domain, int type, int protocol,
                      std::ios_base::openmode which = (std::ios_base::in |
                                                       std::ios_base::out))
  ...
  sockbuf::buffer_type connectto(const struct sockaddr *addr, socklen_t len) {
    return _buf.connectto(addr, len);
  }
