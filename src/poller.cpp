...
#include "io.hpp"
namespace iosched {
poller::size_type poller::_add(native_handle_type handle, events_type &events,
                               event_type event) {
  auto begin = events.begin(), end = events.end();
  auto lb =
      std::lower_bound(begin, end, event, [](const auto &lhs, const auto &rhs) {
...
poller::size_type poller::_update(native_handle_type handle,
                                  events_type &events, event_type event) {
  auto begin = events.begin(), end = events.end();
  auto lb =
      std::lower_bound(begin, end, event, [](const auto &lhs, const auto &rhs) {
        return lhs.fd < rhs.fd;
