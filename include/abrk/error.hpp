 ...
 */
#define ABRK_TOSTRING(x) ABRK_STRINGIFY(x)

/**
 * @def ABRK_ERROR_MESSAGE(msg)
 ...
 *
 * void some_function() {
 *     std::cerr << ABRK_ERROR_MESSAGE("Something went wrong!") << std::endl;
 * }
 * // Possible output:
 * // /path/to/your/file.cpp:123: Something went wrong!
 * @endcode
 */
#define ABRK_ERROR_MESSAGE(msg) (__FILE__ ":" ABRK_TOSTRING(__LINE__) ": " msg)

/**
 * @brief Throws a `std::system_error` with a custom message.
 *
 * This function captures the current `errno`, creates a `std::system_error`
 * with the generic category, and throws it.
 *
