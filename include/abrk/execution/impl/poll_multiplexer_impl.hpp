 ...
 */
#pragma once
#ifndef ABRK_POLL_MULTIPLEXER_IMPL_HPP
#define ABRK_POLL_MULTIPLEXER_IMPL_HPP
#include "abrk/error.hpp"
#include "abrk/execution/detail/utilities.hpp"
...
inline auto handle_poll_error(const std::error_code &error) -> void
{
  if (error != std::errc::interrupted)
    throw_system_error(ABRK_ERROR_MESSAGE("poll failed."));
}
...
inline auto handle_getsockopt_error(std::error_code error) -> int
{
  ...
  {
    throw_system_error(ABRK_ERROR_MESSAGE("getsockopt failed."));
  }
 ...
 */
inline auto set_error(::abrk::socket::socket_handle &socket) -> void
{
  using socket_option = ::abrk::socket::socket_option<int>;

  socket_option error{0};
  auto [ret, optval] = ::abrk::getsockopt(socket, SOL_SOCKET, SO_ERROR, error);
  if (ret)
...
#endif // ABRK_POLL_MULTIPLEXER_IMPL_HPP
