 ...
 */
#pragma once
#ifndef ABRK_ASYNC_OPERATIONS_HPP
#define ABRK_ASYNC_OPERATIONS_HPP
#include "abrk/detail/customization.hpp"
#include "abrk/detail/small_functor.hpp"
#include "abrk/error.hpp"
#include "abrk/execution/detail/execution_trigger.hpp"
#include "abrk/socket/socket_dialog.hpp"
#include "socket.hpp"
    ...
    -> std::shared_ptr<typename socket_dialog<Mux>::executor_type>
{
  ...
  throw std::invalid_argument(ABRK_ERROR_MESSAGE("Invalid executor in dialog."));
}
                ...
                std::span<std::byte> address) -> decltype(auto)
{
  ...
  if constexpr (Mux::template is_eager_v<accept_t>)
  {
    if (++fairness::counter())
    {
      auto [sock, addr] = ::abrk::accept(*socket, address);
      if (sock)
      {
...
{
  return ::abrk::bind(*dialog.socket, address);
}
                ...
                std::span<const std::byte> address) -> decltype(auto)
{
  ...
  using enum abrk::execution::execution_trigger;
  using namespace detail;

  auto executor = get_executor(dialog);
  auto &socket = dialog.socket;

  if (::abrk::connect(*socket, address))
...
{
  return ::abrk::fcntl(*dialog.socket, std::forward<Args>(args)...);
}
...
auto tag_invoke([[maybe_unused]] getpeername_t *ptr,
                ...
                std::span<std::byte> address) -> decltype(auto)
{
  return ::abrk::getpeername(*dialog.socket, address);
}
...
auto tag_invoke([[maybe_unused]] getsockname_t *ptr,
                ...
                std::span<std::byte> address) -> decltype(auto)
{
  return ::abrk::getsockname(*dialog.socket, address);
}
...
{
  return ::abrk::getsockopt(*dialog.socket, level, optname, option);
}
                ...
                int flags) -> decltype(auto)
{
  ...
  if constexpr (Mux::template is_eager_v<recvmsg_t>)
  {
    if (++fairness::counter())
    {
      result_t len = ::abrk::recvmsg(*socket, msg, flags);
  ...
  return executor->set(
      socket, READ, functor([=, socket = socket.get()]() mutable noexcept {
        std::streamsize len = ::abrk::recvmsg(*socket, msghdr, flags);
        if (msg_flags)
                ...
                int flags) -> decltype(auto)
{
  ...
  if constexpr (Mux::template is_eager_v<sendmsg_t>)
  {
    if (++fairness::counter())
    {
      std::streamsize len = ::abrk::sendmsg(*socket, msg, flags | MSG_NOSIGNAL);
  ...
  return executor->set(
      socket, WRITE, functor([=, socket = socket.get()]() noexcept {
        result_t len = ::abrk::sendmsg(*socket, msg, flags | MSG_NOSIGNAL);
        return (len < 0) ? std::nullopt : std::optional<result_t>{len};
      }));
}
...
{
  return ::abrk::listen(*dialog.socket, backlog);
}
...
{
  return ::abrk::setsockopt(*dialog.socket, level, optname, option);
}
...
{
  return ::abrk::shutdown(*dialog.socket, how);
}

} // namespace abrk::socket
#endif // ABRK_ASYNC_OPERATIONS_HPP
